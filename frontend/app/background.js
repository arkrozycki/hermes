...
__webpack_require__.r(__webpack_exports__);
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   cleanupShortcuts: () => (/* binding */ cleanupShortcuts),
/* harmony export */   setupShortcuts: () => (/* binding */ setupShortcuts)
/* harmony export */ });
...
function setupShortcuts() {
  ...
  electron__WEBPACK_IMPORTED_MODULE_0__.globalShortcut.register('CommandOrControl+K', () => {
    const win = electron__WEBPACK_IMPORTED_MODULE_0__.BrowserWindow.getAllWindows()[0];
    if (win) {
...
const getIconPath = () => {
  const theme = electron__WEBPACK_IMPORTED_MODULE_2__.nativeTheme.shouldUseDarkColors ? 'dark' : 'light';
  const platform = process.platform;

  // In development, __dirname points to /path/to/frontend/app/
  // In production, __dirname points to the bundled app location
  const iconBasePath = isProd ? path__WEBPACK_IMPORTED_MODULE_0___default().join(__dirname, '..', 'renderer', 'public', 'icons') : path__WEBPACK_IMPORTED_MODULE_0___default().join(__dirname, '..', 'renderer', 'public', 'icons');
  let iconPath;
  switch (platform) {
    case 'darwin':
      // For macOS, try different formats - .icns for production, PNG for development
      if (isProd) {
        iconPath = path__WEBPACK_IMPORTED_MODULE_0___default().join(iconBasePath, theme, 'desktop', 'macos', 'icon.icns');
      } else {
        // In development, try PNG first as it's more reliable
        const pngPath = path__WEBPACK_IMPORTED_MODULE_0___default().join(iconBasePath, theme, 'desktop', 'linux', 'icon-512x512.png');
        const icnsPath = path__WEBPACK_IMPORTED_MODULE_0___default().join(iconBasePath, theme, 'desktop', 'macos', 'icon.icns');
        iconPath = fs__WEBPACK_IMPORTED_MODULE_1___default().existsSync(pngPath) ? pngPath : icnsPath;
      }
      break;
    case 'win32':
      iconPath = path__WEBPACK_IMPORTED_MODULE_0___default().join(iconBasePath, theme, 'desktop', 'windows', 'icon.ico');
      break;
    default:
      // linux
      iconPath = path__WEBPACK_IMPORTED_MODULE_0___default().join(iconBasePath, theme, 'desktop', 'linux', 'icon-512x512.png');
      break;
  }

  // Debug logging
  console.log('Icon path:', iconPath);
  console.log('Icon exists:', fs__WEBPACK_IMPORTED_MODULE_1___default().existsSync(iconPath));
  console.log('__dirname:', __dirname);
  console.log('Theme:', theme);
  console.log('Platform:', platform);

  // If the theme-specific icon doesn't exist, fall back to the resources folder
  if (!fs__WEBPACK_IMPORTED_MODULE_1___default().existsSync(iconPath)) {
    const fallbackPath = platform === 'darwin' ? path__WEBPACK_IMPORTED_MODULE_0___default().join(__dirname, '..', 'resources', 'icon.icns') : path__WEBPACK_IMPORTED_MODULE_0___default().join(__dirname, '..', 'resources', 'icon.ico');
    console.log('Fallback path:', fallbackPath);
    console.log('Fallback exists:', fs__WEBPACK_IMPORTED_MODULE_1___default().existsSync(fallbackPath));
    if (fs__WEBPACK_IMPORTED_MODULE_1___default().existsSync(fallbackPath)) {
      return fallbackPath;
    }
  }
  return iconPath;
...
(async () => {
  ...
  electron__WEBPACK_IMPORTED_MODULE_2__.app.setName('Hermes');
  ...
  (0,_shortcuts__WEBPACK_IMPORTED_MODULE_5__.setupShortcuts)();

  // Set dock icon for macOS
  if (process.platform === 'darwin' && electron__WEBPACK_IMPORTED_MODULE_2__.app.dock) {
    electron__WEBPACK_IMPORTED_MODULE_2__.app.dock.setIcon(getIconPath());
  }

  // Update icon when system theme changes
  electron__WEBPACK_IMPORTED_MODULE_2__.nativeTheme.on('updated', () => {
    const iconPath = getIconPath();
    mainWindow.setIcon(iconPath);
    if (process.platform === 'darwin' && electron__WEBPACK_IMPORTED_MODULE_2__.app.dock) {
      electron__WEBPACK_IMPORTED_MODULE_2__.app.dock.setIcon(iconPath);
    }
  });
})();
electron__WEBPACK_IMPORTED_MODULE_2__.app.on('window-all-closed', () => {
  (0,_shortcuts__WEBPACK_IMPORTED_MODULE_5__.cleanupShortcuts)();
